      ...
      <nav class="main-menu-container nav nav-pills flex-column sub-open">
        ...
        <ul class="main-menu">
          <li
            ...
            :key="index"
            :class="`${mainmenuItem?.menutitle ? 'slide__category' : ''} ${
          ...
          >
            ...
            <template v-if="mainmenuItem?.type === 'sub'">
              <RecursiveMenu
                ...
                :closeMenuOnNavigate="closeMenuOnNavigate"
                :level="level + 1"
              />
            </template>
          </li>
        ...
        <ul class="doublemenu_bottom-menu main-menu mb-0 border-top">
          ...
          <li class="slide">
            <router-link
              to="/pages/authentication/sign-in/cover"
              class="side-menu__item"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
...
// Create reactive menu data with filtering based on user level
// Initially filter with empty level (will hide restricted items)
const menuData = reactive(filterMenuByLevel(deepClone(staticMenuData), ""));

// Watch user level changes and update menu data
watch(
  userLevel,
  (newLevel) => {
    ...
    menuData.length = 0;
    filteredMenu.forEach((item) => menuData.push(item));
  },
  { immediate: true },
);
...
function closeMenuOnNavigate() {
  // Close dropdown menu when navigating to a child page
  let html = document.documentElement;
  ...
  for (const mutation of mutationsList) {
    if (
      ...
      if (newValue == "vertical") {
        let currentPath = route.path.endsWith("/")
...
onMounted(() => {
  // Fetch user level for menu filtering
  fetchUserLevel();

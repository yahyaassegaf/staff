...
export default defineComponent({
  ...
  setup() {
    ...
    async function getData() {
      try {
        loading.value = true;
        const response = await apiGet("/surat-tugas", {
    ...
    function edit(params: any) {
      router.push(`/st/edit/${params.id}`);
    }

    async function remove(params: any) {
      if (!confirm("Apakah Anda yakin ingin menghapus data ini?")) return;
      try {
        const response = await apiDelete("/surat-tugas/" + params.id);
    ...
    async function download(params: any) {
      try {
        const res = await apiPdf(
          `/surat-tugas/download-pdf/${params.id}`,
          {},
          { responseType: "blob" }
        );
...
<template>
  <div class="container-fluid">
    ...
    >
      <h1 class="page-title fw-semibold fs-18 mb-0">Surat Tugas</h1>
        ...
        <div class="col-md-3 ms-auto">
          <div class="input-group">
            <input
              ...
              v-model="searchValue"
              placeholder="Cari (Dosen/Mhs/NIM)..."
            />

...
export default defineComponent({
  ...
  setup() {
    ...
    async function getData() {
      try {
        loading.value = true;
        const response = await apiGet("/skqa", {
    ...
    function edit(params: any) {
      router.push(`/skqa/edit/${params.id}`);
    }
    ...
    async function remove(params: any) {
      ...
      try {
        const response = await apiDelete("/skqa/" + params.id);
      ...
      <div class="ms-md-1 ms-0">
        <nav>
          <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item active" aria-current="page">SKQA</li>
          </ol>
        </nav>
      </div>
    </div>

...
export default defineComponent({
  ...
  setup() {
    ...
    async function getData() {
      try {
        loading.value = true;
        const response = await apiGet("/surat-keterangan", {
    ...
    async function remove(params: any) {
      ...
      try {
        const response = await apiDelete("/surat-keterangan/" + params.id);
    ...
    async function download(params: any) {
      ...
      try {
        const res = await apiPdf(
          `/surat-keterangan/download-pdf/${params.id}`,
          {},
          { responseType: "blob" }
        );
...
<template>
  <div class="container-fluid">
    ...
    >
      <h1 class="page-title fw-semibold fs-18 mb-0">Surat Keterangan</h1>

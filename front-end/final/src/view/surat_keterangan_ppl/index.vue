...
export default defineComponent({
  ...
  setup() {
    ...
    async function getData() {
      try {
        loading.value = true;
        const response = await apiGet("/skp", {
    ...
    function edit(params: any) {
      ...
      router.push(`/skp/edit/${params.id}`);
    }

    async function remove(params: any) {
      if (!confirm("Apakah anda yakin ingin menghapus data ini?")) return;
      try {
        const response = await apiDelete("/skp/" + params.id);
...
<template>
  <div class="container-fluid">
    ...
    >
      <h1 class="page-title fw-semibold fs-18 mb-0">Surat Keterangan PPL</h1>

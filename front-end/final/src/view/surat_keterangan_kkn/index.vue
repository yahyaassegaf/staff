...
export default defineComponent({
  ...
  setup() {
    ...
    async function getData() {
      try {
        loading.value = true;
        const response = await apiGet("/skk", {
    ...
    function edit(params: any) {
      router.push(`/skk/edit/${params.id}`);
    }
    ...
    async function remove(params: any) {
      ...
      try {
        const response = await apiDelete("/skk/" + params.id);
...
<template>
  <div class="container-fluid">
    ...
    >
      <h1 class="page-title fw-semibold fs-18 mb-0">Surat Keterangan KKN</h1>

...
export default defineComponent({
  ...
  setup() {
    ...
    const headers = [
      ...
      { text: "Penandatangan", value: "nama_penandatangan", sortable: true },
    ...
    async function getData() {
      try {
        loading.value = true;
        const response = await apiGet("/spvn", {
    ...
    function edit(params: any) {
      router.push(`/spvn/edit/${params.id}`);
    }

    async function remove(params: any) {
      if (!confirm("Apakah Anda yakin ingin menghapus data ini?")) return;
      try {
        const response = await apiDelete("/spvn/" + params.id);
  ...
  <div class="container-fluid">
    ...
    >
      <h1 class="page-title fw-semibold fs-18 mb-0">
        Surat Pernyataan Verifikasi Nilai
      </h1>

...
export default defineComponent({
  ...
  setup() {
    ...
    function edit(params: any) {
      router.push(`/tanda-tangan/edit/${params.id}`);
    }
    ...
    async function remove(params: any) {
      ...
      try {
        const response = await apiDelete("/tanda-tangan/" + params.id);
...
<template>
  <div class="container-fluid">
    ...
    >
      <h1 class="page-title fw-semibold fs-18 mb-0">Tanda Tangan Digital</h1>
    ...
    <SimpleCardComponent>
      ...
      >
        ...
        <template #item-gambar="item">
          <img
            v-if="item.gambar"
            :src="BASE_URL.replace('/api', '') + '/' + item.gambar"
            class="img-thumbnail"
            style="height: 50px; background: white"
          />
          <span v-else class="text-muted small">Tidak ada</span>

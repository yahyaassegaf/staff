...
class SuratKeteranganPplControllerTest extends TestCase
{
    protected $user;
    protected $prodi;
    ...
    protected function setUp(): void
    {
        ...
        // 10. Create surat_keterangan_ppl table
        Schema::create('surat_keterangan_ppl', function ($table) {
            $table->id();
    ...
    public function test_index_returns_paginated_daftar_ppl()
    {
        SuratKeteranganPpl::forceCreate([
            'nomor_surat' => 'SU-001/UII.085/TI/TL.00/05/2026',
            'nama_lengkap' => 'Mhs PPL Laki',
        ...
        $response = $this->actingAs($this->user, 'sanctum')->getJson('/api/skp');
    ...
    public function test_index_can_search_daftar_ppl()
    {
        SuratKeteranganPpl::forceCreate([
            'nomor_surat' => 'SU-001/UII.085/TI/TL.00/05/2026',
            'nama_lengkap' => 'Ahmad PPL',
            'tempat_lahir' => 'Pasuruan',
            'tanggal_lahir' => '2000-01-01',
            'nim' => '333444555',
            'prodi_id' => $this->prodi->id,
        ...
        $response = $this->actingAs($this->user, 'sanctum')->getJson('/api/skp?search=Budi');

        $response->assertStatus(200);
        $data = $response->json('data.data');
        $this->assertCount(1, $data);
        $this->assertEquals('Budi PPL', $data[0]['nama_lengkap']);
    }

    public function test_store_creates_daftar_ppl_with_valid_data_and_logs_audit_trail()
    {
        $payload = [
            'prodi_id' => $this->prodi->id,
            'no_surat' => '789',
        ...
        $response = $this->actingAs($this->user, 'sanctum')->postJson('/api/skp', $payload);
        ...
        $this->assertDatabaseHas('surat_keterangan_ppl', [
            'nim' => '999888777',
        ...
        $this->assertDatabaseHas('log_surat', [
            'nomor' => '789',
            'nomor_surat' => $formatted,
            'nama_surat' => 'Surat Keterangan PPL',
            'user_id' => $this->user->id
        ]);
    }
    ...
    public function test_store_fails_with_invalid_data()
    {
        ...
        $response->assertStatus(422)
                 ...
                 ->assertJsonValidationErrors(['no_surat', 'tanda_tangan_id', 'tempat_lahir', 'tanggal_lahir', 'nim', 'prodi_mhs', 'alamat_rumah', 'kelas_pondok', 'tanggal']);
    }

    public function test_show_returns_daftar_ppl()
    {
        $data = SuratKeteranganPpl::forceCreate([
            'nomor_surat' => 'SU-123/UII.085/TI/TL.00/05/2026',
            'nama_lengkap' => 'Show PPL',
            'tempat_lahir' => 'Pasuruan',
            'tanggal_lahir' => '2000-01-01',
            'nim' => '111222333',
            'prodi_id' => $this->prodi->id,
        ...
        $response = $this->actingAs($this->user, 'sanctum')->getJson('/api/skp/' . $data->id);
        ...
        $this->assertEquals('Show PPL', $response->json('data.nama_lengkap'));
    ...
    {
        $response = $this->actingAs($this->user, 'sanctum')->getJson('/api/skp/999');
    ...
    public function test_update_modifies_daftar_ppl_and_logs_audit_trail_when_new_no_surat()
    {
        ...
        $payload = [
            ...
            'nim' => '999999',
            'prodi_mhs' => 'Teknik Informatika',
            'alamat_rumah' => 'Jl. Test PPL Baru',
            'kelas_pondok' => 'B/3',
            'tanggal' => '2026-05-26',
        ];

        $response = $this->actingAs($this->user, 'sanctum')->putJson('/api/skp/' . $data->id, $payload);
    ...
    public function test_update_fails_with_invalid_data()
    {
        ...
        $response->assertStatus(422)
                 ->assertJsonValidationErrors(['nama_mhs', 'no_surat', 'tanda_tangan_id', 'tempat_lahir', 'tanggal_lahir', 'nim', 'prodi_mhs', 'alamat_rumah', 'kelas_pondok', 'tanggal']);
    }
    ...
    public function test_update_returns_404_if_not_found()
    {
        ...
        $response = $this->actingAs($this->user, 'sanctum')->putJson('/api/skp/999', $payload);
    ...
    public function test_destroy_deletes_daftar_ppl()
    {
        $data = SuratKeteranganPpl::forceCreate([
            'nomor_surat' => 'SU-001/UII.085/TI/TL.00/05/2026',
        ...
        $response = $this->actingAs($this->user, 'sanctum')->deleteJson('/api/skp/' . $data->id);
        ...
        $this->assertDatabaseMissing('surat_keterangan_ppl', ['id' => $data->id]);
    }

    public function test_destroy_returns_404_if_not_found()
    {
        $response = $this->actingAs($this->user, 'sanctum')->deleteJson('/api/skp/999');
    ...
    public function test_download_pdf_returns_pdf()
    {
        ...
        $response = $this->get('/api/skp/download-pdf/' . $data->id);

        $response->assertStatus(200);
        $this->assertEquals('application/pdf', $response->headers->get('Content-Type'));
        $this->assertStringContainsString('inline; filename="surat_keterangan_ppl_333444555.pdf"', $response->headers->get('Content-Disposition'));
    }

    public function test_download_pdf_returns_404_if_not_found()
    {
        $response = $this->get('/api/skp/download-pdf/999');
